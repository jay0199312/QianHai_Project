  ...
  <div class="car-message-page">
    ...
    <div class="car-message-item">
      <div class="item-box">
        ...
        <input type="text" placeholder="填写车牌号" id="carNumber" >
        <div class="input-text">车牌号</div>
      </div>
      <div class="item-box2">
        <input type="text" placeholder="填写行驶里程" id="carRun">
...
<script type="text/ecmascript-6">
  ...
  export default {
          ...
          if(carRun.search(run)==-1){
            ...
            if(engine.search(e)==-1){
              ...
              if(frame.search(f)==-1){
                ...
                var myCarAdd = PHPheader + 'api/myCarAdd';
                this.$http.post(myCarAdd,this.getValue,{emulateJSON:true}).then(
                  function (response) {
                    //处理成功的结果
                    this.myCarAdd = response.body;
                    console.log('myCarAdd:',this.myCarAdd);
                    if (this.myCarAdd.status_code =='3'){
                      MessageBox.alert('当前用户下已存在相同车牌号码的车辆！').then(action => {

                      })
                    }if(this.myCarAdd.status_code =='1') {
                      MessageBox.alert('保存车辆成功！').then(action => {
    ...
    created (){
      ...
      this.getValue.goods_id =sessionStorage.getItem("GoodsDetail_ID");
      ...
      this.$http.post(seckGoodsDetail,{goods_id:this.carID},{emulateJSON:true}) //{emulateJSON:true}取数据时候必须传给后台
        .then(response => {
          ...
          this.carName = seckGoodsDetail.goods.name;
